...
const categoriesById = new Map(PRODUCT_CATEGORIES.map((category, displayOrder) => [
  category.id,
...
router.put(
  ...
  async (req, res): Promise<void> => {
    ...
    const now = new Date();
    const categoryImageUrl = categoryImagePath
      ? await createFirebaseDownloadUrl(categoryImagePath)
      : null;
    const values = {
      categoryId: category.id,
      categoryName: category.name,
